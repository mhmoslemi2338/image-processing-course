...
def apply_homography(image, pnt):
    [w1, h1] = [(np.linalg.norm(pnt[0] - pnt[1])),
                (np.linalg.norm(pnt[1] - pnt[2]))]
    [w2, h2] = [(np.linalg.norm(pnt[2] - pnt[3])),
                (np.linalg.norm(pnt[3] - pnt[0]))]
...
pnt1 = np.array([[385, 105], [317, 288], [601, 394],
                [666, 209]])  # graph theory book
pnt2 = np.array([[410, 469], [205, 427], [153, 709], [363, 741]])  # DFT book
pnt3 = np.array([[622, 667], [420, 795], [609, 1100],
                [813, 968]])  # image science book
